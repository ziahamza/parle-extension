...
1,254 unit tests · 20/20 typecheck · e2e 63/63 · torture 48/48 · 21 ADRs
```
...
```bash
pnpm install
pnpm check                         # typecheck + tests: 20/20, 1,254 unit tests
...
split quality/package checks, the 63-check browser suite, the 48-check torture suite, and a real Apple
...
| command | what it is |
|---|---|
| `pnpm e2e` | **the gate.** 63 behaviour checks: consent-before-anything, what went on the wire, what is on disk, the mark, the in-page panel on every surface, adaptive navigation geometry, the Digest, the Safari-shaped overlay |
