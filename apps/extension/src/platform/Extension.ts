...
import {
  clearRecentOpenings as clearRecentOpeningsCommand,
  type RecentOpening,
  type RecentOpeningCommand
} from "../history/RecentOpening.ts"

/** The containing application generated by `package-safari.sh`. */
const SAFARI_APPLICATION = "com.ziahamza.parle"
const NATIVE_MESSAGE_TIMEOUT_MS = 5_000

// Safari does not expose cancellation for sendNativeMessage. A timeout can
// therefore settle our caller while the native request is still running. Keep
// this worker's underlying requests in order; the native store's commutative
// merge supplies the same guarantee across service-worker restarts.
let nativeMessageTail: Promise<void> = Promise.resolve()

const sendNativeMessageInOrder = (command: RecentOpeningCommand): Promise<unknown> => {
  const sent = nativeMessageTail.then(
    () => browser.runtime.sendNativeMessage(SAFARI_APPLICATION, command),
    () => browser.runtime.sendNativeMessage(SAFARI_APPLICATION, command)
  )
  nativeMessageTail = sent.then(() => undefined, () => undefined)
  return sent
}

const nativeMessage = (command: RecentOpeningCommand): Promise<unknown> =>
  new Promise((resolve, reject) => {
    const timer = setTimeout(
      () => reject(new Error("Safari native message timed out")),
      NATIVE_MESSAGE_TIMEOUT_MS
    )
    void sendNativeMessageInOrder(command).then(
      (reply) => {
        clearTimeout(timer)
        resolve(reply)
      },
...
export class Extension extends Context.Service<Extension, {
  ...
  /** Whether this build has Safari's containing-app Recent store. */
  readonly supportsRecentOpenings: boolean
  /** Mirror one user-explicit panel opening and report the native acknowledgement. */
  readonly recordRecentOpening: (opening: RecentOpening) => Effect.Effect<boolean>
  /** Remove mirrored openings no newer than the reader's clear gesture. */
  readonly clearRecentOpenings: (clearedAt: number) => Effect.Effect<boolean>
  /**
  ...
  static readonly layerFrom = (
    ...
    Effect.gen(function*() {
      ...
      /**
       * The containing app is a best-effort mirror while a panel is painting,
       * but a destructive clear must be able to distinguish an acknowledged
       * native write from a transport or storage failure. The warning names
       * only the operation — never the command, whose Subject URL is reader
       * history and must not be copied into a log.
       */
      const sendToSafari = Effect.fn("Extension.sendToSafari")(function*(
        command: RecentOpeningCommand,
        operation: "record an opening" | "clear recent openings"
      ) {
        if (!safari) return true
        const ok = yield* Effect.tryPromise(() => nativeMessage(command)).pipe(
          Effect.map((response: unknown) =>
            typeof response === "object" && response !== null &&
            (response as { readonly ok?: unknown }).ok === true
          ),
          Effect.catch(() => Effect.succeed(false))
        )
        if (!ok) yield* Effect.logWarning(`parle: Safari companion could not ${operation}`)
        return ok
      })

      const recordRecentOpening = (opening: RecentOpening): Effect.Effect<boolean> =>
        sendToSafari(opening, "record an opening")

      const clearRecentOpenings = (clearedAt: number): Effect.Effect<boolean> =>
        sendToSafari(clearRecentOpeningsCommand(clearedAt), "clear recent openings")
