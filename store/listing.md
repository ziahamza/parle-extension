...
| Version | unknown | `1.0.0` (set in `apps/extension/wxt.config.ts`) |
...
`apps/extension/.output/chrome-mv3/manifest.json`. As built at version `1.0.0` the declared
...
- [ ] **Version must exceed the version already on the item.** It is set to `1.0.0` in
      `apps/extension/wxt.config.ts`. The taken-down MV2 package's version is not knowable from
      this repo — the console shows it. If the upload is rejected with *"version number must be
      greater than…"*, raise the value above whatever the console names and rebuild. Nothing
      else in the submission depends on the number.
